...
const QUICK_GRAMS = [
  { label: '0.1gm', value: 0.1, popular: false },
  { label: '0.5gm', value: 0.5, popular: false },
  { label: '1gm', value: 1, popular: false },
  { label: '1.5gm', value: 1.5, popular: false },
  { label: '2gm', value: 2, popular: true },
  { label: '3gm', value: 3, popular: false },
];

export default function BuySilverScreen() {
  const router = useRouter();
  const [buyMode, setBuyMode] = useState<'Rupees' | 'Grams'>('Grams');
  ...
  const converted =
    buyMode === 'Rupees'
      ? `${(numVal / SILVER_RATE).toFixed(4)} gm`
      : `₹${(numVal * SILVER_RATE).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`;
    ...
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      >
        ...
        <View style={styles.heroSection}>
          ...
          <Text style={styles.heroBig}>BUY 999 SILVER</Text>

          {/* Rate row */}
          <View style={styles.rateRow}>
            <Text style={styles.rateLabel}>Market Buy Rate</Text>
            <View style={styles.rateDot} />
            <Text style={styles.rateValue}>₹265.62/g</Text>
          </View>
          ...
          {(buyMode === 'Rupees' ? QUICK_AMOUNTS : QUICK_GRAMS).map((item) => (
            ...
            >
              ...
              {item.popular && (
                <Text style={[
                  styles.popularTag,
                  String(item.value) === inputValue && styles.popularTagActive,
                ]}>
                  ★ Popular
                </Text>
              )}
